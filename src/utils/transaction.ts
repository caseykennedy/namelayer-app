...
export function getTXValue(tx: Transaction | SignMessageRequest): number {
  ...
  for (let i = 0; i < tx.outputs.length; i++) {
    ...
    if (output.path && output.path.change) continue;
    ...
    if (covAction && covenant.action !== covAction) continue;
  ...
  const inputValue = tx.inputs.reduce((sum: number, input: any) => {
    if (input.coin) {
  ...
  const outputValue = tx.outputs.reduce((sum: number, output: any) => {
